  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Document Sync</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filtered.map(doc => {
          ...
          return (
            <div
              key={doc.path}
              className="bg-white rounded-2xl shadow-card border border-gray-100 p-5 hover:shadow-card-hover hover:scale-[1.02] transition-all duration-200 cursor-default group"
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="rounded-lg bg-gray-100 p-2.5 group-hover:bg-teal/10 transition-colors duration-200">
                  <FileText size={18} className="text-gray-500 group-hover:text-teal transition-colors duration-200" />
                </div>
              ...
              {/* Doc ID */}
              <h3 className="font-semibold text-sm text-gray-900 truncate">{doc.doc_id}</h3>

              {/* Path */}
              <p className="text-xs text-gray-400 mt-1 truncate" title={doc.path}>{doc.path}</p>
              ...
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
                ...
                <span className="font-mono text-[11px] font-semibold px-2 py-0.5 rounded-md bg-gray-100 text-gray-500">
                  {doc.standard}
                </span>
              </div>
            </div>
